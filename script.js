...
(function heavyDutyRepair() {
    ...
    panel.innerHTML = `
        <div class="mode-row">
            <button class="mode-btn" id="m-btn-1" onclick="forceSetMode(1)">🍃 佛系减压</button>
            <button class="mode-btn active-mode" id="m-btn-2" onclick="forceSetMode(2)">⚖️ 标准</button>
            <button class="mode-btn" id="m-btn-3" onclick="forceSetMode(3)">🔥 魔鬼应试</button>
        </div>
    ...
    window.forceSetMode = function(m) {
        ...
        const tips = ["已开启佛系减压模式", "已恢复标准模式", "已进入魔鬼应试模式"];
        addLog(`系统：${tips[m-1]}`);
    };
    ...
    window.forceNextRound = function() {
        ...
        if(typeof nextRound === 'function') {
            // 如果原来的函数还在，尝试运行它
            try { nextRound(); } catch(e) { console.log("原结算函数有损，执行强制补救结算"); }
        }

        // --- 核心模式修正 ---
        if (state.mode === 1) {
            // 佛系模式：强制大幅扣除压力，不管其他建筑加了多少
            state.pressure = Math.max(0, state.pressure - 30);
            addLog("🧘 [佛系生效] 全校开展心理建设，压力大幅消散！");
        } else if (state.mode === 3) {
            // 应试模式：额外增加压力
            state.pressure = Math.min(100, state.pressure + 20);
            addLog("👿 [魔鬼生效] 疯狂刷题中，压力快速上升！");
        }

        updateUI();
        addLog(`>>> 第 ${state.round - 1} 学期结算完毕 <<<`);
    };

    // 5. 自动隐藏旧的无效按钮 (如果它们还残留在页面上的话)
