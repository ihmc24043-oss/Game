...
(function rebuildSystem() {
    ...
    if (typeof state === 'undefined') {
        window.state = { 
            money: 1000000, pressure: 0, reputation: 60, students: 20, 
            capacity: 50, round: 1, mode: 2, teachers: [], slots: Array(24).fill(null) 
        };
    }
    ...
    style.innerHTML = `
        #rebuild-panel {
            ...
            background: #2c3e50; padding: 15px 0; z-index: 10000;
            display: flex; flex-direction: column; align-items: center;
            border-top: 3px solid #f1c40f; box-shadow: 0 -5px 20px rgba(0,0,0,0.5);
        }
        .rebuild-modes { display: flex; gap: 10px; margin-bottom: 12px; }
        .mode-btn-new {
            padding: 8px 15px; border-radius: 20px; border: 1px solid #7f8c8d;
            background: #34495e; color: #ecf0f1; font-size: 13px; cursor: pointer;
        }
        .mode-btn-new.active {
            background: #3498db; color: white; border-color: #fff; font-weight: bold;
        }
        .mode-btn-new.fo-mode { border-color: #27ae60; }
        .mode-btn-new.fo-mode.active { background: #27ae60; }
        
        .next-round-btn {
            width: 90%; padding: 15px; background: #f1c40f; color: #2c3e50;
            border: none; border-radius: 30px; font-size: 18px; font-weight: bold;
            box-shadow: 0 4px 0 #d4ac0d;
        }
        .next-round-btn:active { transform: translateY(2px); box-shadow: 0 2px 0 #d4ac0d; }
        
        /* 隐藏旧的冲突 UI */
        .controls, .mode-toggles { display: none !important; }
    `;
    document.head.appendChild(style);

    // 3. 强行在 Body 挂载全新的 UI 浮层
    const panel = document.createElement('div');
    panel.id = 'rebuild-panel';
    panel.innerHTML = `
        <div class="rebuild-modes">
            <button class="mode-btn-new fo-mode" id="mb-1" onclick="sysSetMode(1)">🍃 佛系(减压)</button>
            <button class="mode-btn-new" id="mb-2" onclick="sysSetMode(2)">⚖️ 标准</button>
            <button class="mode-btn-new" id="mb-3" onclick="sysSetMode(3)">🔥 应试(提分)</button>
        </div>
        <button class="next-round-btn" onclick="sysNextRound()">🔔 进入下一学期结算</button>
    `;
    document.body.appendChild(panel);

    // 4. 重做：模式切换逻辑
    window.sysSetMode = function(m) {
        state.mode = m;
        document.querySelectorAll('.mode-btn-new').forEach((b, i) => {
    ...
    // 5. 重做：学期结算逻辑 (彻底解决压力问题)
    window.sysNextRound = function() {
        // 自动补全必要条件 (防止报错无法开始)
        if (state.teachers.length < 1) {
            addLog("系统：自动为你招募了基础教职工...");
            for(let i=0; i<6; i++) state.teachers.push({name:"临时工", salary:2000, power:10});
        }

        // 基础逻辑运算
        let studyGain = state.teachers.reduce((a, b) => a + b.power, 0);
        let roomStudy = state.slots.reduce((a, b) => a + (b ? (b.study || 0) : 0), 0);
        let roomPressure = state.slots.reduce((a, b) => a + (b ? (b.pressure || 0) : 0), 0);

        // --- 核心：模式修正 ---
        if (state.mode === 1) { // 佛系
            state.pressure = Math.max(0, state.pressure - 30 + (roomPressure * 0.5));
            state.reputation += (studyGain + roomStudy) * 0.05;
            addLog("🧘 [佛系减压] 学生们去爬山了，压力大减！");
        } else if (state.mode === 3) { // 应试
            state.pressure = Math.min(100, state.pressure + 20 + (roomPressure * 1.5));
            state.reputation += (studyGain + roomStudy) * 0.3;
            addLog("👿 [应试魔鬼] 狂刷卷子，声望大涨，压力狂飙！");
        } else { // 标准
            state.pressure = Math.max(0, Math.min(100, state.pressure + roomPressure + 5));
            state.reputation += (studyGain + roomStudy) * 0.15;
            addLog("⚖️ [标准模式] 平稳度过了一个学期。");
        }

        // 财务
        let income = state.students * (state.fee || 5000);
        let outgo = state.teachers.reduce((a, b) => a + b.salary, 0);
        state.money += (income - outgo);
        ...
        if(typeof updateUI === 'function') updateUI();
            ...
            // 如果 updateUI 被删了，强行更新显示
            const mDisp = document.getElementById('money');
            if(mDisp) mDisp.innerText = `¥${state.money.toLocaleString()}`;
            const pDisp = document.getElementById('pressure');
            if(pDisp) pDisp.innerText = `${Math.floor(state.pressure)}%`;
        }
    };
